...
static void charging_information(struct work_struct *work)
{
	...
	pr_cc(PR_INFO, "USB_PRESENT(%d), WIRELESS_PRESENT(%d), USB_TYPE(%s), CABLE_INFO(%s), USBIN_VOL(%d), "
		"BATT_FAKE(%s), BATT_TEMP(%d), BATT_SOC(%d), BATT_VOL(%d), "
		"PMI_IUSB_SET(%d), PMI_IUSB_AICL(%d), PMI_IBAT_SET(%d), TOTAL_IBAT_NOW(%d)\n",
...
#define DEFAULT_FAKE_BATT_VOLTAGE		0
static int get_prop_fuelgauge_voltage(void)
{
	union power_supply_propval val = {0, };

#ifdef CONFIG_LGE_PM_FACTORY_PSEUDO_BATTERY
	if (pseudo_batt_info.mode) {
		return pseudo_batt_info.volt * 1000;
	}
#endif
	...
	if (chgr_contr->fg_psy == NULL) {
		...
		if (chgr_contr->fg_psy == NULL) {
			pr_err("[ChargerController]########## Not Ready(fg_psy)\n");
			return DEFAULT_FAKE_BATT_VOLTAGE;
		}
	}
	/* unit : uV */
	if (!cc_psy_getprop(fg_psy, VOLTAGE_NOW, &val))
		return val.intval;
	else
		return DEFAULT_FAKE_BATT_VOLTAGE;
}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = get_prop_fuelgauge_voltage();
		pr_cc(PR_DEBUG, "Battery Voltage = %d\n", val->intval);
		break;
