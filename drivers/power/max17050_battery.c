...
static struct max17050_chip *the_chip;
...
int max17050_get_full_design(void)
{
	if (!the_chip)
		...
		return the_chip->pdata->full_design;
}
...
/*	if ((usb_cable_info == 6) ||
	...
	/*End of the Custom Model step 1.*/
	pr_debug("[MAX17050] End of the max17050_force_custom_model_write.\n");
...
/*	if(lge_bd_rev > LGE_REV_C) kwon temporary*/
	{
		/*Call max17050_force_custom_model_write*/
...
int max17050_get_battery_full_design(void)
{
	return max17050_get_full_design();
}
EXPORT_SYMBOL(max17050_get_battery_full_design);
			    ...
			    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		val->intval = max17050_get_battery_full_design();
		break;
...
#ifdef CONFIG_LGE_PM
	pdata->alert_gpio = of_get_named_gpio(dev_node,
			"max17050,alert_gpio", 0);
	pr_info("%s : Get alert_gpio %d\n",
		 __func__, pdata->alert_gpio);

	rc = of_property_read_u32(dev_node, "max17050,full_design",
			&pdata->full_design);

	pr_info("[MAX17050] platform data : full_design = %d\n", pdata->full_design);
#endif
			...
			const struct i2c_device_id *id)
{
	...
	chip->pdata = devm_kzalloc(&client->dev,
					 ...
					 GFP_KERNEL);
	if (!chip->pdata) {
		pr_err("[MAX17050] %s : missing platform data\n", __func__);
		return -ENODEV;
	}
	...
	if (ret != 0) {
		pr_err("[MAX17050] %s : device tree parsing error\n", __func__);
		return ret;
	}
#else
	...
	ret = power_supply_register(&chip->client->dev, &max17050_ps);
	if (ret < 0)
		pr_err("[MAX17050]failed to register a power_supply rc = %d\n"
			, ret);
