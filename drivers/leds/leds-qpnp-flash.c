...
enum flash_led_id {
	FLASH_LED_0 = 0,
	FLASH_LED_1,
//	FLASH_LED_SWITCH,/* LGE rollback migration */
};

enum flash_led_type {
	FLASH = 0,
	TORCH,
//	SWITCH,/* LGE rollback migration */
};
...
struct flash_node_data {
	...
	u16				duration;/* LGE rollback migration */
	u16				max_current;
	u16				current_addr;/* LGE rollback migration */
	u16				prgm_current;
	u8				id;
	u8				type;
	u8				trigger;
...
static u8 qpnp_flash_led_ctrl_dbg_regs[] = {
	...
	0x4A, 0x4B, 0x4F, 0x51, 0x52, 0x54, 0x55, 0x5A/* LGE rollback migration */
};
/* LGE rollback migration */
#if 0
static int
...
static int qpnp_flash_led_module_disable(struct qpnp_flash_led *led,
				struct flash_node_data *flash_node)
{
//	union power_supply_propval psy_prop;/* LGE rollback migration */
	int rc;
	u8 val, tmp;
	...
	if (!tmp) {
		if (flash_node->type == TORCH) {/* LGE rollback migration */
			rc = qpnp_led_masked_write(led->spmi_dev,
...
#endif/* LGE rollback migration */
static void qpnp_flash_led_brightness_set(struct led_classdev *led_cdev,
						enum led_brightness value)
{
	struct flash_node_data *flash_node;
//	struct qpnp_flash_led *led;/* LGE rollback migration */
	flash_node = container_of(led_cdev, struct flash_node_data, cdev);
//	led = dev_get_drvdata(&flash_node->spmi_dev->dev);/* LGE rollback migration */
					...
					struct flash_node_data *flash_node)
{
	...
	if (!rc) {
		if (strcmp(temp_string, "flash") == 0)
			...
			flash_node->type = TORCH;
//		else if (strcmp(temp_string, "switch") == 0)/* LGE rollback migration */
//			flash_node->type = SWITCH;
	...
	if (!rc) {
		...
		flash_node->prgm_current = (u16)val;/* LGE rollback migration */
	} else if (rc != -EINVAL) {
		dev_err(&led->spmi_dev->dev,
...
/* LGE rollback migration */
	...
	case FLASH_SUBTYPE_SINGLE:
		flash_node->current_addr = FLASH_LED0_CURRENT(led->base);/* LGE rollback migration */
...
//	led->current2_addr = FLASH_LED1_CURRENT(led->base);
	...
	for_each_child_of_node(node, temp) {
		...
		} else if (rc < 0) {/* LGE rollback migration */
			dev_err(&led->spmi_dev->dev,
...
module_init(qpnp_flash_led_init);/* LGE rollback migration */
