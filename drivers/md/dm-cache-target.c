			   ...
			   dm_cblock_t cblock)
{
	...
	if (!block_size_is_power_of_two(cache))
		bio->bi_iter.bi_sector = (from_cblock(cblock) * cache->sectors_per_block) +
				sector_div(bi_iter.bi_sector, cache->sectors_per_block);
	else
		bio->bi_iter.bi_sector = (from_cblock(cblock) << cache->sectors_per_block_shift) |
				(bi_iter.bi_sector & (cache->sectors_per_block - 1));
}
...
static void process_discard_bio(struct cache *cache, struct bio *bio)
{
	...
	dm_block_t end_block = bio->bi_iter.bi_sector + bio_sectors(bio);
	dm_block_t b;
