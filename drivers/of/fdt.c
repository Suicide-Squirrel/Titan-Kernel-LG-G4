...
// the compiler threw an error about strchrnul, so just use this functionally equivalent substitute
static const char *_strchrnul(const char *str, int chr) {
	const char *ptr = strchr(str, chr);
	if (!ptr)
		ptr = str + strlen(str);
	return ptr;
}
...
static bool __init do_command_line_searchreplace(char *command_line, unsigned size)
{
	...
	for(i=0; i<ARRAY_SIZE(cmdline_searchreplace_list); ++i) {
		...
		if(search[slen-1]!=' ' && replace[rlen-1]==' ')
			mlen=_strchrnul(match+mlen, ' ')-match+1;
