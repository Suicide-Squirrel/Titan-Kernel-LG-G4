...
#include <linux/spi/fpc_btp.h>
#include <linux/spi/fpc_btp_regs.h>
...
static int fpc_btp_probe(struct spi_device *spi)
{
	...
	if(fpc_btp_reset_init(fpc_btp, fpc_btp_pdata))
		goto err;

	if(fpc_btp_irq_init(fpc_btp, fpc_btp_pdata))
		goto err;

	if(fpc_btp_spi_setup(fpc_btp, fpc_btp_pdata))
		goto err;
	...
	if(fpc_btp_verify_hw_id(fpc_btp))
		goto err;
