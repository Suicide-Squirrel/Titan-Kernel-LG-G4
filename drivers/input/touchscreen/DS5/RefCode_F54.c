...
static int FrameDelta_C[32][18];
static int FrameDelta_M[32][18];
static int FrameDelta_P[32][18];
static int cnt = 0;
static int max_frame_delta = 0;
...
int RspReadImageReport_frame(void)
{
	int k = 0;
	int i = 0;
	int j = 0;
	...
	f54len = 0;
	TOUCH_I("Frame Delta Test Start\n");
	f54len += snprintf(f54buf + f54len,
			sizeof(f54buf) - f54len,
			"Frame Rawdata Test Result\n");

	do {
		if (cnt < 3){
			TOUCH_I("frame waste : %d\n", cnt);
		} else {
			...
			count = 0;
			memset(FrameDelta_C, 0, sizeof(FrameDelta_C));
			Read8BitRegisters(F54DataBase + REPORT_DATA_OFFEST,
					&Data[0], MaxArrayLength);
			...
			for (i = 0; i < TxChannelCount; i++) {
				for (j = 0; j < RxChannelCount; j++) {
					Image1[i][j] = (int)Data[k] | ((int)Data[k + 1] << 8);
					if ((i < 32) && (j < 18)) {
						FrameDelta_C[i][j] = Image1[i][j];
						if (cnt > 3) {
							diff = f_sub(FrameDelta_C[i][j], FrameDelta_P[i][j]);
							if (diff > FrameDelta_M[i][j])
								FrameDelta_M[i][j] = diff;
						}
					}
					k = k + 2;
				}
		...
		memcpy(FrameDelta_P,FrameDelta_C, sizeof(FrameDelta_C));
	} while (cnt < 50);

	max_frame_delta = FrameDelta_M[0][0];
	f54len += snprintf(f54buf + f54len,
			sizeof(f54buf) - f54len,
			"==========================================================================================================\n         :");
	...
	for (i = 0; i < TxChannelCount; i++) {
		f54len += snprintf(f54buf + f54len,sizeof(f54buf) - f54len,"   %5d : ", i);
			for (j = 0; j < RxChannelCount; j++) {
				if ((i < 32) && (j < 18)) {
					f54len += snprintf(f54buf + f54len,
						sizeof(f54buf) - f54len,
						"%5d ",FrameDelta_M[i][j]);
						if (max_frame_delta < FrameDelta_M[i][j]){
							max_frame_delta = FrameDelta_M[i][j];
							max_i = i;
							max_j = j;
							TOUCH_I("max_frame_delta = [%d][%d]%d\n", i, j, max_frame_delta);
				}
			}
		}
		f54len += snprintf(f54buf + f54len,
	...
	TOUCH_I("Frame Delta Test End : %d\n", max_frame_delta);

	if (max_frame_delta > 30)
		ret = 0;

	if (!ret) {
		TOUCH_I("Frame Rawdata Test failed\n");
		f54len += snprintf(f54buf + f54len,
				sizeof(f54buf) - f54len,
				"max_frame_rawdata = [%d][%d]%d\n",max_i,max_j,max_frame_delta);
		f54len += snprintf(f54buf + f54len,
				sizeof(f54buf) - f54len,
				"Frame Rawdata Test failed.\n");
	} else {
		TOUCH_I("Frame Rawdata Test passed\n");
		f54len += snprintf(f54buf + f54len,
				sizeof(f54buf) - f54len,
				"Frame Rawdata Test passed.\n");
	}
	f54len += snprintf(f54buf + f54len,
			sizeof(f54buf) - f54len,
			"\n");
	...
	cnt = 0;
	max_frame_delta = 0;
	memset(FrameDelta_M, 0, sizeof(FrameDelta_M));
	memset(FrameDelta_C, 0, sizeof(FrameDelta_C));
...
int GroupNormalTest(int mode, char *buf)
{
	...
	switch(mode){
		...
		case RSP_LPWG_RAW_DATA:
			...
			RspCollectRawData();
			break;
		case RSP_FRAME_DELTA:
			ret = RspReadImageReport_frame();
			break;
		default:
			TOUCH_I("[%s] mode = %d\n", __func__, mode);
			break;
