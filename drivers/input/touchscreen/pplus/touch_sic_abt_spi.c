...
static void abt_ksocket_start_for_pctool(struct spi_device *client)
{
...
static void abt_ksocket_start_for_abtstudio(struct spi_device *client)
{
	int size, err;
	int bufsize = 10;
...
static void abt_recv_packet_parsing(struct spi_device *spi,
				...
				struct s_comm_packet *p_packet_to_pc)
{
	...
	if (p_packet_from_pc->flag.rw == READ_TYPE) {
		...
		if (p_packet_from_pc->data_size % I2C_PACKET_SIZE != 0) {
			ret = sic_spi_write(spi, addr_ptr,
						data_ptr_from_pc,
						p_packet_from_pc->data_size %
						I2C_PACKET_SIZE);
		}
...
static int abt_set_report_mode(struct spi_device *client, u32 mode)
{
	int ret;
	TOUCH_D(DEBUG_BASE_INFO,
		"[ABT](%s)mode:%d\n", __func__, mode);
...
static int32_t abt_ksocket_init(struct spi_device *client,
			char *ip,
...
ssize_t show_abtApp(struct spi_device *client, char *buf)
{
	int i;
	ssize_t retVal = 0;
...
ssize_t store_abtApp(struct spi_device *client, const char *buf, size_t count)
{
	u32 mode;
...
ssize_t show_abtTool(struct spi_device *client, char *buf)
{
...
ssize_t store_abtTool(struct spi_device *client,
				const char *buf,
				size_t count)
{
	int mode = buf[0];
...
int abt_force_set_report_mode(struct spi_device *client, u32 mode)
{
	int ret = 0;
	u32 rdata = 0;
	u32 wdata = mode;
	...
	if (mode >= 0) {
		ret = sic_spi_read(client,
					CMD_RAW_DATA_REPORT_MODE_READ,
					(u8 *)&rdata,
					sizeof(rdata));
		TOUCH_D(DEBUG_BASE_INFO, "(%d)rdata\n",  rdata);
		if (ret < 0 || rdata <= 0) {
			TOUCH_D(DEBUG_BASE_INFO,
