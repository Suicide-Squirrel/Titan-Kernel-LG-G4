	       ...
	       int is_no_sleep)
{
	...
	if (ctrl_pdata->lge_pan_data->dsv_manufacturer == DSV_SM5107){
		...
		if (!mode) { /* NEAR :  LPWG -> DEEP SLEEP */
			...
			sm5107_lgd_fd_mode_change(1); /* fd_on */
		...
		if (!gpio_get_value(ctrl_pdata->lge_pan_data->dsv_ena)) {
			if (gpio_is_valid(ctrl_pdata->lge_pan_data->dsv_ena)) {
				...
				sm5107_lgd_fd_mode_change(0); /* fd_off */
				...
				ctrl_pdata->power_data[DSI_CTRL_PM].num_vreg, 1); //Power on

				mdss_dsi_clk_ctrl(ctrl_pdata, DSI_ALL_CLKS, 1); // clk on
				mdss_dsi_clk_ctrl(ctrl_pdata, DSI_ALL_CLKS, 0); // clk off

				rc = msm_dss_enable_vreg(
				ctrl_pdata->power_data[DSI_CTRL_PM].vreg_config,
				ctrl_pdata->power_data[DSI_CTRL_PM].num_vreg, 0);//Power off
		...
		if (!mode) { /* NEAR :  LPWG -> DEEP SLEEP */
			...
			if (gpio_is_valid(ctrl_pdata->lge_pan_data->dsv_ena)) {
					...
					PANEL_SEQUENCE(PIN_DSV_ENA, 0);
					gpio_free(ctrl_pdata->lge_pan_data->dsv_ena);
					//pr_info("%s: mode : %d ena# 2 = %d\n", __func__, mode, gpio_get_value(ctrl_pdata->lge_pan_data->dsv_ena));
				}
				if (ctrl_pdata->lge_pan_data->touch_driver_registered)
					touch_notifier_call_chain(
							LCD_EVENT_TOUCH_SLEEP_STATUS,
	...
	switch(cur_mode)
	{
		...
		case READER_MODE_MONO:
			...
			if (reader_mode_step2_cmds.cmd_cnt) {
			        pr_info("%s: mono reader_mode_step2_cmds: cnt = %d \n",
...
{
	struct mdss_dsi_ctrl_pdata *other;
	other = mdss_dsi_get_other_ctrl(ctrl);
		...
		switch(old_mode)
		{
			...
			case READER_MODE_STEP_3:
				if(reader_mode_off_cmds.cmd_cnt) {
					...
					mdss_dsi_panel_cmds_send(other, &reader_mode_off_cmds, CMD_REQ_COMMIT);
				...
				switch(new_mode)
				{
					case READER_MODE_STEP_1:
						if(reader_mode_step1_cmds.cmd_cnt) {
							...
							mdss_dsi_panel_cmds_send(other, &reader_mode_step1_cmds, CMD_REQ_COMMIT);
					...
					case READER_MODE_STEP_2:
						if(reader_mode_step2_cmds.cmd_cnt) {
							...
							mdss_dsi_panel_cmds_send(other, &reader_mode_step2_cmds, CMD_REQ_COMMIT);
						}
						break;
					case READER_MODE_STEP_3:
						if (reader_mode_step3_cmds.cmd_cnt) {
							...
							mdss_dsi_panel_cmds_send(other, &reader_mode_step3_cmds, CMD_REQ_COMMIT);
						}
						break;
					case READER_MODE_MONO:
						// D-IC does not support MONO Mode
						if (reader_mode_step2_cmds.cmd_cnt) {
							pr_info("%s: mono sending reader mode STEP2 commands: cnt : %d \n",
								__func__, reader_mode_step2_cmds.cmd_cnt);
							mdss_dsi_panel_cmds_send(ctrl, &reader_mode_step2_cmds, CMD_REQ_COMMIT);
							mdss_dsi_panel_cmds_send(other, &reader_mode_step2_cmds, CMD_REQ_COMMIT);
						}
