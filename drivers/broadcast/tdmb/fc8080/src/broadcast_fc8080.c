...
static int tunerbb_drv_load_hw_configure(struct spi_device *spi)
{
    ...
    if(get_board_revision() < HW_REV_0)
    {
    ...
    {
        fc8080_ctrl_info.dmb_use_xtal = false;
		fc8080_ctrl_info.dmb_xtal_freq = 19200;
    }
    else
    {
        fc8080_ctrl_info.dmb_use_xtal = true;
		fc8080_ctrl_info.dmb_xtal_freq = 27120;
    }

    printk("[dbg] rev no(%d) & chip_select(%d) & dmb_use_xtal(%d)\n", get_board_revision(), fc8080_ctrl_info.spi_ptr->chip_select, fc8080_ctrl_info.dmb_use_xtal);
#endif
