...
int sm5107_fast_discharge(void)
{
	int ret = 0;
	...
	pr_err("sm5107 Fast Discharge.\n");
	ret = p1_dsv_write_byte(p1_dsv_base, 0x03, 0x03);
	msleep(20);
	ret = p1_dsv_write_byte(p1_dsv_base, 0x03, 0x00);
	return ret;
}
EXPORT_SYMBOL_GPL(sm5107_fast_discharge);
...
int sm5107_mode_change(int mode)
{
	...
	if (mode == 0)
		...
		ret = p1_dsv_write_byte(p1_dsv_base, SM5107_CTRL_SET_REG, 0x28);

	return ret;
}
EXPORT_SYMBOL_GPL(sm5107_mode_change);

int sm5107_lgd_dsv_setting(int enable)
{
int ret = 0;
	...
	if (enable == 2) {
		ret += p1_dsv_write_byte(p1_dsv_base, 0x03, 0x03);
		ret += p1_dsv_write_byte(p1_dsv_base, 0xFF, 0x00); //Normal mode
		pr_info("SM DSV FD on set, ret = %d\n", ret);
	} else if (enable == 1) {
		ret  = p1_dsv_write_byte(p1_dsv_base, 0x00, 0x0F);
		ret += p1_dsv_write_byte(p1_dsv_base, 0x01, 0x0F);
		ret += p1_dsv_write_byte(p1_dsv_base, 0x03, 0x03);
		ret += p1_dsv_write_byte(p1_dsv_base, 0xFF, 0x28);
		pr_info("SM DSV Normal mode set, ret = %d\n", ret);
	} else {
		ret  = p1_dsv_write_byte(p1_dsv_base, 0x00, 0x09);
		ret += p1_dsv_write_byte(p1_dsv_base, 0x01, 0x09);
		ret += p1_dsv_write_byte(p1_dsv_base, 0x03, 0x00);
		ret += p1_dsv_write_byte(p1_dsv_base, 0xFF, 0x28);
		pr_info("SM DSV LPWG mode set, ret = %d\n", ret);
	}
	return ret;
}
EXPORT_SYMBOL_GPL(sm5107_lgd_dsv_setting);

int sm5107_lgd_fd_mode_change(int enable)
{
	int ret = 0;
	...
	if (enable) {
		...
		ret += p1_dsv_write_byte(p1_dsv_base, 0x03, 0x03);
		ret += p1_dsv_write_byte(p1_dsv_base, 0xFF, 0x28);
		pr_info("SM5107_lgd_fd_mode_change enable : %d, ret = %d\n", enable, ret);
...
EXPORT_SYMBOL_GPL(sm5107_lgd_fd_mode_change);
...
int dw8768_lgd_dsv_setting(int enable)
{
	...
	if (enable == 2) {
		...
		ret  = p1_dsv_write_byte(p1_dsv_base, 0x00, 0x0F);
#if defined(CONFIG_MACH_MSM8992_SJR)
		ret += p1_dsv_write_byte(p1_dsv_base, 0x01, 0x13); //touch ghost issue -5.9v
#else
		ret += p1_dsv_write_byte(p1_dsv_base, 0x01, 0x0F);
#endif 
		ret += p1_dsv_write_byte(p1_dsv_base, 0x03, 0x87);
